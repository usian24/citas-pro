...
function setupWorkerPhotoUpload() {
  var coverInp = G('wk-profile-cover-input'); if (coverInp) {
    coverInp.addEventListener('change', async function (e) {
      var f = e.target.files[0]; if (!f || !validImageType(f)) { toast('Solo JPG/PNG/WebP (máx 5MB)', '#EF4444'); return; }
      toast('Optimizando y subiendo...', '#F59E0B');
      var optimizedFile = await processImageForUpload(f);
      var url = await uploadToImgBB(optimizedFile);
      if (url && CUR_WORKER) { CUR_WORKER.cover = url; renderWorkerProfile(); toast('Portada subida, presiona Guardar', '#22C55E'); }
    });
  }
  var logoInp = G('wk-profile-photo-input'); if (logoInp) {
    logoInp.addEventListener('change', async function (e) {
      var f = e.target.files[0]; if (!f || !validImageType(f)) { toast('Solo JPG/PNG/WebP (máx 5MB)', '#EF4444'); return; }
      toast('Optimizando y subiendo...', '#F59E0B');
      var optimizedFile = await processImageForUpload(f);
      var url = await uploadToImgBB(optimizedFile);
      if (url && CUR_WORKER) { CUR_WORKER.photo = url; renderWorkerProfile(); toast('Foto subida, presiona Guardar', '#22C55E'); }
    });
  }
  var galInp = G('wk-gallery-input'); if (galInp) {
    galInp.addEventListener('change', async function (e) {
      ...
      for (var i = 0; i < files.length; i++) {
        var f = files[i]; if (!validImageType(f)) continue;
        var optimizedFile = await processImageForUpload(f);
        var url = await uploadToImgBB(optimizedFile);
        if (url && CUR_WORKER) { if (!CUR_WORKER.photos) CUR_WORKER.photos = []; if (CUR_WORKER.photos.length >= 20) { toast('Máximo 20 fotos', '#EF4444'); return; } CUR_WORKER.photos.push(url); saveDB(); renderWorkerGallery(); }
      } toast('Fotos subidas', '#22C55E');
    });
  }
  var svcInp = G('wk-sv-photo-input'); if (svcInp) {
    svcInp.addEventListener('change', async function (e) {
      var f = e.target.files[0]; if (!f || !validImageType(f)) return;
      toast('Optimizando y subiendo...', '#F59E0B');
      var optimizedFile = await processImageForUpload(f);
      var url = await uploadToImgBB(optimizedFile);
      if (url) { window._wkSvcPhoto = url; var pv = G('wk-sv-photo-preview'); if (pv) pv.innerHTML = '<img src="' + url + '" class="photo-preview" alt="Servicio"/>'; toast('Foto lista', '#22C55E'); }
    });
  }
}
...
function renderWorkerDailyTimeline(dateStr) {
  ...
  container.classList.remove('wk-split-timeline');'use strict';
  ...
  function showWorkerPanel() {
    if (!DB.currentWorker) { goTo('s-portal'); return; }
    const biz = getBizById(DB.currentWorker.bizId);
    const worker = getWorkerById(DB.currentWorker.bizId, DB.currentWorker.workerId);
    if (!biz || !worker) { goTo('s-portal'); return; }
  ...
  function initWorkerPanel() {
    ...
    const bizName = CUR ? (CUR.name || 'Mi Negocio') : 'Mi Negocio';
    const bizAv = G('wk-biz-av');
    if (bizAv) {
      if (CUR && CUR.logo) bizAv.innerHTML = '<img src="' + sanitizeImageDataURL(CUR.logo) + '" style="width:100%;height:100%;object-fit:cover" alt="Logo"/>';
      else bizAv.textContent = bizName.charAt(0).toUpperCase();
    }
    ...
    const firstName = (CUR_WORKER.name || 'Trabajador').split(' ')[0];
    T('wk-greeting', '¡Hola, ' + firstName + '!');
  
    /* Stats home — usando timezone del negocio */
    const _hoyDate = (typeof ahoraEnNegocio === 'function') ? ahoraEnNegocio(CUR.country || 'PE') : new Date();
    const today = _hoyDate.getFullYear() + '-' + String(_hoyDate.getMonth() + 1).padStart(2, '0') + '-' + String(_hoyDate.getDate()).padStart(2, '0');
    const appts = CUR_WORKER.appointments || [];
    const todayA = appts.filter(function (a) { return a.date === today && a.status !== 'cancelled'; });
  
    const thisWeekStart = new Date(_hoyDate); thisWeekStart.setDate(_hoyDate.getDate() - _hoyDate.getDay());
    ...
    const thisMonthStart = new Date(_hoyDate); thisMonthStart.setDate(1);
    const thisMonthStartStr = thisMonthStart.getFullYear() + '-' + String(thisMonthStart.getMonth() + 1).padStart(2, '0') + '-01';
  
    const weekA = appts.filter(function (a) { return a.date >= thisWeekStartStr && a.status !== 'cancelled'; });
    const monthA = appts.filter(function (a) { return a.date >= thisMonthStartStr && a.status !== 'cancelled'; });
    ...
    // ── Banner de notificaciones (solo primera vez, si nunca dio permiso) ──
    if ('Notification' in window && Notification.permission === 'default') {
      const bannerKey = 'cp_push_banner_' + CUR_WORKER.id;
      if (!localStorage.getItem(bannerKey)) {
  ...
  function openWorkerConfig() {
    openOv('ov-config');
    const tgl = G('toggle-push-notifs');
    if (tgl) {
      if (Notification.permission === 'granted') tgl.classList.add('on');
      else tgl.classList.remove('on');
    }
  ...
  ══════════════════════════ */
  function workerApptRowH(a) {
    const sc = { confirmed: { c: 'var(--blue)', bg: 'rgba(74,127,212,.1)', l: 'Conf.' }, rescheduled: { c: '#F59E0B', bg: 'rgba(245,158,11,.15)', l: 'Reag.' }, in_progress: { c: '#A855F7', bg: 'rgba(168,85,247,.15)', l: 'En curso' }, pending: { c: 'var(--gold)', bg: 'rgba(245,158,11,.1)', l: 'Pend.' }, completed: { c: 'var(--green)', bg: 'rgba(34,197,94,.1)', l: 'Hecho' }, cancelled: { c: 'var(--red)', bg: 'rgba(239,68,68,.1)', l: 'Canc.' } }[a.status] || { c: 'var(--blue)', bg: 'rgba(74,127,212,.1)', l: 'Conf.' };
  ...
  function openWorkerApptDetail(id) {
    ...
    const loyaltyHtml = typeof buildLoyaltyHtml === 'function' ? buildLoyaltyHtml(a, allAppts) : '';
  
    const waLink = a.phone ? 'https://wa.me/' + a.phone.replace(/\D/g, '') + '?text=' + encodeURIComponent('Hola ' + a.client + ', te escribo por tu cita en ' + (CUR ? CUR.name : 'nuestro local') + ' el ' + a.date + ' a las ' + a.time + '.') : '#';
  ...
  /* ══════════════════════════
     AGENDA TRABAJADOR
  ══════════════════════════ */
  let workerCalDate = new Date();
  let workerCalDay = new Date().toISOString().split('T')[0];
  
  function renderWorkerCalendar() {
    const now = workerCalDate, year = now.getFullYear(), month = now.getMonth();
    T('wk-cal-title', MONTHS[month] + ' ' + year);
    const firstDay = new Date(year, month, 1).getDay(), daysInMonth = new Date(year, month + 1, 0).getDate();
  
    // Usar timezone del negocio
    const _hoy = (typeof ahoraEnNegocio === 'function' && CUR) ? ahoraEnNegocio(CUR.country || 'PE') : new Date();
    const today = _hoy.getFullYear() + '-' + String(_hoy.getMonth() + 1).padStart(2, '0') + '-' + String(_hoy.getDate()).padStart(2, '0');
  
    const appts = CUR_WORKER ? (CUR_WORKER.appointments || []) : [], apptDates = {};
    appts.forEach(function (a) { if (a.date && a.status !== 'cancelled') apptDates[a.date] = true; });
    let html = '';
  ...
  /* ══════════════════════════
     SERVICIOS TRABAJADOR
  ══════════════════════════ */
  let editWorkerSvc = null;
  
  function renderWorkerServices() {
    if (!CUR_WORKER) return;
    const svcs = CUR_WORKER.services || [];
    if (svcs.length) {
      H('wk-svcs-list', svcs.map(function (s) {
        const thumb = s.photo ? '<img src="' + sanitizeImageDataURL(s.photo) + '" style="width:46px;height:46px;border-radius:11px;object-fit:cover;flex-shrink:0" alt="Servicio">'
          : '<div style="width:46px;height:46px;border-radius:11px;background:var(--bblue);color:var(--blue);display:flex;align-items:center;justify-content:center;flex-shrink:0"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20.59 13.41l-7.17 7.17a2 2 0 0 1-2.83 0L2 12V2h10l8.59 8.59a2 2 0 0 1 0 2.82z"></path><line x1="7" y1="7" x2="7.01" y2="7"></line></svg></div>';
        return '<div style="background:var(--card);border:1px solid var(--b);border-radius:20px;padding:14px;display:flex;align-items:center;gap:12px;margin-bottom:10px">'
          + thumb + '<div style="flex:1"><div style="font-weight:700;font-size:14px">' + san(s.name) + '</div>'
          + '<div style="font-size:12px;color:var(--muted);margin-top:2px">' + s.dur + 'min' + (s.desc ? ' · ' + san(s.desc) : '') + '</div></div>'
  ...
  function openWorkerSvcModal(id) {
    editWorkerSvc = id || null; window._wkSvcPhoto = null;
    T('wk-svc-ttl', id ? 'Editar servicio' : 'Añadir servicio');
    const reset = function () { const p = G('wk-sv-photo-preview'); if (p) p.innerHTML = '<div style="font-size:13px;color:var(--muted)">Añadir foto</div>'; };
    if (id && CUR_WORKER) {
      const s = (CUR_WORKER.services || []).filter(function (x) { return String(x.id) === String(id); })[0];
      if (s) {
        const n = G('wk-sv-name'), pr = G('wk-sv-price'), dr = G('wk-sv-dur'), ds = G('wk-sv-desc');
        if (n) n.value = s.name; if (pr) pr.value = s.price; if (dr) dr.value = s.dur; if (ds) ds.value = s.desc || '';
        const pv = G('wk-sv-photo-preview'); if (pv && s.photo) pv.innerHTML = '<img src="' + sanitizeImageDataURL(s.photo) + '" class="photo-preview" alt="Servicio"/>'; else reset();
      }
    } else {
      ['wk-sv-name', 'wk-sv-price', 'wk-sv-desc'].forEach(function (i) { const e = G(i); if (e) e.value = ''; });
      const dv = G('wk-sv-dur'); if (dv) dv.value = '30'; reset();
    }
    openOv('ov-wk-svc');
  }
  
  function saveWorkerSvc() {
    const name = sanitizeText(V('wk-sv-name')), price = safeNum(V('wk-sv-price'), 0), dur = safeInt(V('wk-sv-dur'), 30), desc = sanitizeText(V('wk-sv-desc')), photo = window._wkSvcPhoto || null;
    if (!name) { toast('Nombre requerido', '#EF4444'); return; }
    if (!CUR_WORKER) return; if (!CUR_WORKER.services) CUR_WORKER.services = [];
    if (editWorkerSvc) { const s = CUR_WORKER.services.filter(function (x) { return String(x.id) === String(editWorkerSvc); })[0]; if (s) { s.name = name; s.price = price; s.dur = dur; s.desc = desc; if (photo) s.photo = photo; } }
    else CUR_WORKER.services.push({ id: 'ws_' + Date.now(), name: name, price: price, dur: dur, desc: desc, photo: photo || '' });
    editWorkerSvc = null; window._wkSvcPhoto = null; saveDB(); renderWorkerServices(); closeOv('ov-wk-svc'); toast('Servicio guardado', '#4A7FD4');
  }
  ...
  function renderWorkerGallery() {
    if (!CUR_WORKER) return;
    const grid = G('wk-gallery'); if (!grid) return;
    grid.innerHTML = (CUR_WORKER.photos || []).map(function (p, i) {
      return '<div class="img-thumb"><img src="' + sanitizeImageDataURL(p) + '" alt="Foto ' + (i + 1) + '"><button onclick="delWorkerGalleryPhoto(' + i + ')" style="position:absolute;top:4px;right:4px;background:rgba(0,0,0,.65);border:none;border-radius:5px;color:#fff;font-size:11px;padding:2px 6px;cursor:pointer">×</button></div>';
    }).join('');
  }
  ...
  function renderWorkerFinances() {
    ...
    const appts = CUR_WORKER.appointments || [];
    const active = appts.filter(function (a) { return a.status !== 'cancelled'; });
    const paid = appts.filter(function (a) { return a.status !== 'cancelled' && a.price > 0; });
    ...
    const cliCount = {}; active.forEach(function (a) { if (a.client) cliCount[a.client] = (cliCount[a.client] || 0) + 1; });
    let topCli = '—', topCC = 0; Object.keys(cliCount).forEach(function (k) { if (cliCount[k] > topCC) { topCli = k; topCC = cliCount[k]; } });
    ...
    const wVals = weeks.map(function (ws) {
      const we = new Date(ws + 'T12:00'); we.setDate(we.getDate() + 6); const weStr = we.toISOString().split('T')[0];
      return active.filter(function (a) { return a.date && a.date >= ws && a.date <= weStr; }).reduce(function (s, a) { return s + (a.price || 0); }, 0);
    });
    ...
    mVals.forEach(function (v, i) {
      const h = Math.max(4, Math.round(v / mMax * 100)), isLast = i === mVals.length - 1;
      // ✅ AQUÍ ESTÁ LA CORRECCIÓN
      html += '<div style="flex:1; height:100%; display:flex; flex-direction:column; justify-content:flex-end; align-items:center; gap:3px">';
      if (v > 0) html += '<div style="font-size:8px;color:var(--muted)">' + money(v) + '</div>';
      html += '<div style="width:100%;height:' + h + '%;border-radius:5px 5px 0 0;background:' + (isLast ? 'linear-gradient(to top,var(--blue2),var(--blue3))' : 'linear-gradient(to top,rgba(74,127,212,.3),rgba(74,127,212,.5))') + '" title="' + money(v) + '"></div>';
    ...
    html += '</div><div style="display:flex;gap:5px">';
    months.forEach(function (m, i) { const p = m.split('-'); html += '<div style="flex:1;text-align:center;font-size:9px;color:' + (i === months.length - 1 ? 'var(--blue)' : 'var(--muted)') + ';font-weight:700">' + MONTHS_SHORT[parseInt(p[1]) - 1] + '</div>'; });
    html += '</div></div>';
    ...
    wVals.forEach(function (v, i) {
      const h = Math.max(4, Math.round(v / wMax * 100)), isLast = i === wVals.length - 1;
      // ✅ AQUÍ ESTÁ LA CORRECCIÓN
      html += '<div style="flex:1; height:100%; display:flex; flex-direction:column; justify-content:flex-end; align-items:center; gap:3px">';
      if (v > 0) html += '<div style="font-size:8px;color:var(--muted)">' + money(v) + '</div>';
      html += '<div style="width:100%;height:' + h + '%;border-radius:5px 5px 0 0;background:' + (isLast ? 'linear-gradient(to top,#16A34A,#4ADE80)' : 'linear-gradient(to top,rgba(34,197,94,.25),rgba(34,197,94,.5))') + '" title="' + money(v) + '"></div>';
    ...
    const el = G('wp-finanzas'); if (el) el.innerHTML = html;
  
    // Generar historial exclusivamente en su propia pestaña
    const historial = appts.slice().sort(function (a, b) {
      return (b.date || '').localeCompare(a.date || '');
    }).slice(0, 100);
    const histHtml = historial.length ? historial.map(function (a) { return workerApptRowH(a); }).join('') : '<div style="text-align:center;padding:24px;color:var(--muted);font-size:13px">Sin registros</div>';
    const histEl = G('wk-appts-fin'); if (histEl) histEl.innerHTML = histHtml;
  }
  ...
  function getHorarioSeguro() {
    ...
    const diasPlantilla = plantilla.map(function (p) { return p.day; });
    diasPlantilla.forEach(function (dia, idx) {
      let existente = CUR_WORKER.horario.filter(function (h) { return h.day === dia; })[0];
      if (!existente) { CUR_WORKER.horario.push(Object.assign({}, plantilla[idx])); }
      else {
        if (existente.from1 === undefined) existente.from1 = plantilla[idx].from1;
        if (existente.to1 === undefined) existente.to1 = plantilla[idx].to1;
  ...
  function renderWorkerHorario() {
    if (!CUR_WORKER) return;
    const horario = getHorarioSeguro();
    H('wk-horario-days', horario.map(function (day, i) {
      const f1 = day.from1 || day.from || '09:00', t1 = day.to1 || day.to || '14:00', hb = !!day.hasBreak, f2 = day.from2 || '16:00', t2 = day.to2 || '20:00';
      let content = '';
      if (day.open) {
        content = '<div style="margin-top:12px">'
          + '<div style="display:flex;gap:10px;align-items:center;margin-bottom:12px">'
  ...
  window.toggleWorkerBreak = function (i) {
    if (!CUR_WORKER || !CUR_WORKER.horario || !CUR_WORKER.horario[i]) return;
    const h = CUR_WORKER.horario[i]; h.hasBreak = !h.hasBreak;
    if (h.hasBreak && !h.from2) { h.from2 = '16:00'; h.to2 = '20:00'; } if (!h.hasBreak) { h.from2 = ''; h.to2 = ''; }
    renderWorkerHorario();
  };
  ...
  function renderWorkerProfile() {
    if (!CUR_WORKER) return;
    const nm = G('wk-pf-nm'), ph = G('wk-pf-phone'), sp = G('wk-pf-spec'), em = G('wk-pf-email');
    if (nm) nm.value = CUR_WORKER.name || ''; if (ph) ph.value = CUR_WORKER.phone || ''; if (sp) sp.value = CUR_WORKER.spec || ''; if (em) em.value = CUR_WORKER.email || '';
    const profileCover = G('wk-profile-cover'); if (profileCover && CUR_WORKER.cover) profileCover.style.backgroundImage = 'url(' + sanitizeImageDataURL(CUR_WORKER.cover) + ')';
    const pv = G('wk-profile-photo-preview');
    if (pv) { if (CUR_WORKER.photo) pv.innerHTML = '<img src="' + sanitizeImageDataURL(CUR_WORKER.photo) + '" style="width:100%;height:100%;object-fit:cover;border-radius:50%" alt="Foto"/>'; else pv.innerHTML = '<span style="font-size:32px;font-weight:800;color:#fff">' + (CUR_WORKER.name || '?').charAt(0).toUpperCase() + '</span>'; }
    T('wk-pf-biz-name', CUR ? CUR.name : '');
    T('wk-pf-biz-addr', CUR ? ((CUR.addr || '') + ' ' + (CUR.city || '')) : '');
  }
  ...
  function saveWorkerProfile() {
    if (!CUR_WORKER) return;
    const nm = sanitizeText(V('wk-pf-nm'));
    if (!nm) { toast('El nombre no puede estar vacío', '#EF4444'); return; }
    CUR_WORKER.name = nm; CUR_WORKER.phone = sanitizeText(V('wk-pf-phone')); CUR_WORKER.spec = sanitizeText(V('wk-pf-spec'));
    syncWorkerToCloud(); saveDB(); initWorkerPanel(); toast('Perfil guardado', '#4A7FD4');
  }
  ...
  function saveWorkerPassword() {
    const p1 = V('wk-pass-new'), p2 = V('wk-pass-confirm'); hideErr('wk-pass-err');
    if (!p1 || p1.length < 6) { showErr('wk-pass-err', 'Mínimo 6 caracteres.'); return; }
    if (p1 !== p2) { showErr('wk-pass-err', 'Las contraseñas no coinciden.'); return; }
    if (!CUR_WORKER) return;
    CUR_WORKER.pass = p1; syncWorkerToCloud(); saveDB();
    const f1 = G('wk-pass-new'), f2 = G('wk-pass-confirm'); if (f1) f1.value = ''; if (f2) f2.value = '';
    toast('Contraseña actualizada', '#22C55E');
  }
  ...
  async function saveWorkerHorario() {
    ...
    const btn = G('save-wk-horario-btn');
  ...
  ══════════════════════════ */
  function setupWorkerPhotoUpload() {
    const coverInp = G('wk-profile-cover-input');
    ...
    const logoInp = G('wk-profile-photo-input');
    ...
    const galInp = G('wk-gallery-input');
    ...
    const svcInp = G('wk-sv-photo-input');
  ...
  function renderWorkerNotifBadge() {
    if (!CUR_WORKER) return;
    const unread = (CUR_WORKER.notifications || []).filter(function (n) { return !n.read; }).length;
    const b1 = G('wn-notif-badge'), b2 = G('wk-notif-badge');
    if (b1) { b1.style.display = unread > 0 ? 'flex' : 'none'; b1.textContent = unread; }
    if (b2) { b2.style.display = unread > 0 ? 'inline-block' : 'none'; b2.textContent = unread; }
  }
  ...
  function renderWorkerNotifications() {
    if (!CUR_WORKER) return;
    const notifs = CUR_WORKER.notifications || [];
    if (notifs.length) {
      H('wk-notif-list', notifs.map(function (n, i) {
        const bg = n.read ? 'transparent' : 'rgba(74,127,212,.08)', border = n.read ? 'var(--b)' : 'var(--blue)';
        return '<div style="padding:16px;border-bottom:1px solid var(--b);background:' + bg + ';border-left:3px solid ' + border + ';margin-bottom:8px;border-radius:0 12px 12px 0">'
          + '<div style="display:flex;justify-content:space-between;margin-bottom:6px"><span style="font-weight:800;font-size:14px;color:' + (n.read ? 'var(--text)' : 'var(--blue)') + '">' + san(n.title) + '</span><span style="font-size:11px;color:var(--muted)">' + san(n.date) + '</span></div>'
          + '<div style="font-size:13px;color:var(--t2);line-height:1.5">' + san(n.body) + '</div>'
          + (!n.read ? '<div style="text-align:right;margin-top:8px"><button onclick="markWorkerNotifRead(' + i + ')" style="background:var(--bblue);border:none;color:var(--blue);font-size:11px;font-weight:700;cursor:pointer;padding:6px 12px;border-radius:12px">Marcar como leída</button></div>' : '')
    ...
    const clearBtn = G('clear-notif-btn');
    if (clearBtn) { clearBtn.onclick = function () { openConfirmModal('Limpiar Notificaciones', '¿Borrar todas las notificaciones?', function () { CUR_WORKER.notifications = []; saveDB(); renderWorkerNotifications(); renderWorkerNotifBadge(); toast('Notificaciones borradas', '#475569'); }); }; }
  }
  
  function markWorkerNotifRead(index) {
    if (!CUR_WORKER || !CUR_WORKER.notifications || !CUR_WORKER.notifications[index]) return;
    const n = CUR_WORKER.notifications[index];
  ...
  function initWorkerAgenda() {
    if (!CUR_WORKER) return;
    const parts = workerCalDay.split('-'), days = ['Domingo', 'Lunes', 'Martes', 'Miércoles', 'Jueves', 'Viernes', 'Sábado'], d = new Date(workerCalDay + 'T12:00');
    const lbl = G('wk-agenda-day-label');
    if (lbl) { lbl.textContent = days[d.getDay()] + ' ' + parseInt(parts[2]) + ' de ' + MONTHS[parseInt(parts[1]) - 1] + ' de ' + parts[0]; lbl.style.textAlign = 'center'; lbl.style.fontSize = '14px'; }
    const listEl = G('wk-agenda-list'); if (listEl) listEl.innerHTML = '';
    renderWorkerDailyTimeline(workerCalDay);
  }
  
  function renderWorkerDailyTimeline(dateStr) {
    const container = G('wk-daily-timeline'); if (!container || !CUR_WORKER) return;
    const pxPerMin = 1.5;
    ...
    try {
      ...
      const wh = (CUR_WORKER.horario || []).find(function (h) { return h.day === dayName; });
      if (wh && wh.open) {
        const dayStarts = [];
        const dayEnds = [];
        const f1 = wh.from1 || wh.from;
        const t1 = wh.to1 || wh.to;
        if (f1 && t1) {
          dayStarts.push(parseInt(String(f1).split(':')[0], 10));
          dayEnds.push(parseInt(String(t1).split(':')[0], 10));
        }
    ...
    // 2) Ajustar por citas del día (si existen)
    const wAppts = (CUR_WORKER.appointments || []).filter(function (a) { return a.date === dateStr && a.status !== 'cancelled'; });
    wAppts.forEach(function (a) {
      const parts = String(a.time || '00:00').split(':');
      ...
      let dur = 30;
      if (CUR_WORKER.services && CUR_WORKER.services.length) {
        const svc = CUR_WORKER.services.find(function (s) { return s.name === a.svc; });
        if (svc) dur = parseInt(svc.dur, 10) || 30;
      }
      const endMins = (h * 60 + m) + dur;
      const endHourAppt = Math.ceil(endMins / 60);
    ...
    if (endHour - startHour < 8) {
    const targetEnd = Math.min(23, startHour + 8);
      endHour = Math.max(endHour, targetEnd);
    }
  
    function buildTimelineSegment(segStartHour, segEndHour, title, apptsOffsetIndex) {
    const totalHeight = (segEndHour - segStartHour) * 60 * pxPerMin;
    const segAppts = wAppts.filter(function (a) {
      const parts = String(a.time || '00:00').split(':');
      const h = parseInt(parts[0], 10);
        return !isNaN(h) && h >= segStartHour && h < segEndHour;
      });
  
    let segHtml = '<div class="tl-wrap"><div class="tl-grid"><div class="tl-times"><div class="tl-header"></div><div class="tl-body" style="height:' + totalHeight + 'px;background:none;">';
    for (let hh = segStartHour; hh <= segEndHour; hh++) segHtml += '<div class="tl-time-lbl" style="top:' + ((hh - segStartHour) * 60 * pxPerMin) + 'px">' + String(hh).padStart(2, '0') + ':00</div>';
      segHtml += '</div></div><div class="tl-col"><div class="tl-header"><div style="font-size:12px;font-weight:800;color:var(--blue)">' + title + '</div></div><div class="tl-body" style="height:' + totalHeight + 'px;background-size:100% ' + (60 * pxPerMin) + 'px;">';
      if (typeof generateBlockedTimeHTML === 'function') segHtml += generateBlockedTimeHTML(CUR_WORKER, dateStr, segStartHour, segEndHour, pxPerMin);
      segAppts.forEach(function (a, i) {
        if (typeof generateTimelineApptHTML === 'function') {
          segHtml += generateTimelineApptHTML(a, CUR_WORKER, segStartHour, pxPerMin, (apptsOffsetIndex || 0) + i, 'openWorkerApptDetail');
        }
      });
      segHtml += '</div></div></div></div>';
      return segHtml;
  ...
  function showFloatingDeleteButton() {
    ...
    if (!btn) {
      ...
      btn.innerHTML = `
        ...
        <div onclick="exitSelectMode()" style="margin-top:10px;font-size:16px;font-weight:bold;color:var(--text);background:var(--card);padding:25px 12px;border-radius:12px;box-shadow:0 4px 10px rgba(9, 24, 231, 0.18);cursor:pointer;">Cancelar</div>
      `;
  ...
  window.openWorkerRescheduleModal = function(id) {
    closeOv('ov-wk-appt-detail');
    const appt = (CUR_WORKER.appointments || []).find(function(a) { return String(a.id) === String(id); });
    if (!appt) return;
  
    window._rescheduleApptId = id;
    window._rescheduleSelectedTime = null;
  
    let ov = document.getElementById('ov-wk-reschedule');
    if (!ov) {
  ...
  window.renderWorkerRescheduleTimes = function() {
    const appt = (CUR_WORKER.appointments || []).find(function(a) { return String(a.id) === String(window._rescheduleApptId); });
    if (!appt) return;
  
    const dateStr = document.getElementById('wk-resched-date').value;
    const timesEl = document.getElementById('wk-resched-times');
    if (!dateStr) {
    ...
    if (CUR_WORKER.services) {
      const sObj = CUR_WORKER.services.find(function(s) { return s.name === appt.svc; });
      if (sObj) dur = parseInt(sObj.dur) || 30;
    }
  
    const d = new Date(dateStr + 'T12:00');
    const dayName = ['Domingo','Lunes','Martes','Miércoles','Jueves','Viernes','Sábado'][d.getDay()];
    const hDay = (CUR_WORKER.horario || []).find(function(h) { return h.day === dayName; }) || {open:true, from1:'09:00', to1:'14:00'};
    ...
    (CUR_WORKER.appointments || []).forEach(function(a) {
      if (a.date === dateStr && a.status !== 'cancelled' && String(a.id) !== String(appt.id)) {
        let sDur = 30;
        const sObj = (CUR_WORKER.services||[]).find(function(s) { return s.name === a.svc; });
  ...
  window.confirmWorkerReschedule = async function() {
    ...
    const newDate = document.getElementById('wk-resched-date').value;
    const newTime = window._rescheduleSelectedTime;
    const apptId = window._rescheduleApptId;
  
    const appt = (CUR_WORKER.appointments || []).find(function(a) { return String(a.id) === String(apptId); });
    if (!appt) return;
