...
const ICONOS_CAT = {
  ...
  locion: `<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="4" y="2" width="16" height="20" rx="2" ry="2"></rect><circle cx="12" cy="14" r="4"></circle><line x1="12" y1="6" x2="12.01" y2="6"></line></svg>`,
  corte: `<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="6" cy="6" r="3"></circle><circle cx="6" cy="18" r="3"></circle><line x1="20" y1="4" x2="8.12" y2="15.88"></line><line x1="14.47" y1="14.48" x2="20" y2="20"></line><line x1="8.12" y1="8.12" x2="12" y2="12"></line></svg>`,
  barba: `<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 13c0 5 4 9 9 9s9-4 9-9"></path><path d="M12 22a8.5 8.5 0 0 0 7-5.5"></path><path d="M5 16.5A8.5 8.5 0 0 1 12 22"></path><path d="M9 9h6"></path><path d="M12 6v3"></path></svg>`,
  gel: `<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M10 2v7.31"></path><path d="M14 9.3V1.99"></path><path d="M8.5 2h7"></path><path d="M14 9.3a6.5 6.5 0 1 1-4 0"></path><path d="M5.52 16h12.96"></path></svg>`,
  equipo: `<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="3" width="18" height="18" rx="2" ry="2"></rect><circle cx="8.5" cy="8.5" r="1.5"></circle><polyline points="21 15 16 10 5 21"></polyline></svg>`
};
...
async function renderTiendaAdmin() {
  ...
  if (CUR.categories.length === 0) {
    ...
    catList.innerHTML = CUR.categories.map(c => {
      ...
      return `
      <div onclick="editCategory('${c.id}')" style="background:var(--card);border:1px solid var(--b);border-radius:16px;padding:8px 14px;display:flex;align-items:center;gap:8px;flex-shrink:0;box-shadow:0 4px 6px rgba(0,0,0,0.05);cursor:pointer;transition:transform 0.2s;">
        <div style="color:var(--blue);display:flex;align-items:center;background:rgba(74,127,212,0.1);padding:6px;border-radius:50%;">${sv}</div>
        <span style="font-size:13px;font-weight:700;color:var(--text);">${c.name}</span>
        <button onclick="deleteCategory('${c.id}', event)" style="color:var(--red);font-size:16px;margin-left:6px;font-weight:bold;background:none;border:none;cursor:pointer;padding:4px;">×</button>
      </div>
    `}).join('');
  }
  ...
  try {
    ...
    prodList.innerHTML = productosSupabase.map(p => {
      ...
      <div onclick="editProduct('${p.id}')" style="background:#fff;border:1px solid var(--b);border-radius:16px;padding:10px;position:relative;display:flex;flex-direction:column;box-shadow:0 4px 10px rgba(0,0,0,0.05);cursor:pointer;transition:transform 0.2s;">
        ...
        <div style="font-size:10px;color:var(--blue);font-weight:800;text-transform:uppercase;display:flex;align-items:center;gap:4px;">
           <span style="width:14px;height:14px;">${catIcon}</span> ${catName}
        </div>
...
function editCategory(id) {
  let cat = CUR.categories.find(c => c.id === id);
  if (!cat) return;
  
  editingCatId = id;
  document.getElementById('cat-name').value = cat.name;
  document.getElementById('cat-icon-sel').value = cat.iconCode || 'equipo';
  openOv('ov-tienda-cat');
}
...
function openProdModal() {
  if (!CUR.categories || CUR.categories.length === 0) {
    mostrarAlertaTienda('¡Primero debes crear al menos una categoría!', 'Aviso', '📂');
    return;
  }
  editingProdId = null;
